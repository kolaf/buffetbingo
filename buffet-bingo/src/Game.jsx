...
const ScoreForm = ({ lobbyId, user, onComplete, currentName }) => {
  ...
  const submitScore = async () => {
    ...
    try {
      ...
      const storageRef = ref(storage, `plates/${lobbyId}/${user.uid}.jpg`);
      await uploadBytes(storageRef, photo);
      const photoUrl = await getDownloadURL(storageRef);

      await setDoc(doc(db, "lobbies", lobbyId, "players", user.uid), {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <button onClick={onComplete} className="mb-6 text-slate-500 hover:text-slate-800 flex items-center transition">
        <i className="fas fa-arrow-left mr-2"></i> Back to Lobby
      </button>
...
function Game() {
  ...
  useEffect(() => {
    ...
    // Listen to the 'players' sub-collection inside the lobby
  ...
  const createLobby = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "lobbies", code), { host: user.uid, createdAt: new Date() });

      // Add self to sub-collection
      console.log("Adding self to sub-collection");
      await setDoc(doc(db, "lobbies", code, "players", user.uid), {
  ...
  const joinLobby = async (e) => {
    ...
    if (lobbySnap.exists()) {
      setLobbyId(code);
      const playerRef = doc(db, "lobbies", code, "players", user.uid);
            ...
            {!lobbyId ? (
                ...
                <div className="max-w-md mx-auto mt-10">
                    <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 text-center">
                        ...
                        <p className="text-slate-500 mb-8">Create a room or join your friends.</p>
